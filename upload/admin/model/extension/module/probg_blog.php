...
class ModelExtensionModuleProbgBlog extends Model {
    public function install() {
        ...
        if ($this->config->get('module_probg_blog_version') === null) {
            $this->model_setting_setting->editSetting('module_probg_blog', array('module_probg_blog_status'=>0,'module_probg_blog_sort'=>'date','module_probg_blog_limit'=>10,'module_probg_blog_image_list_width'=>400,'module_probg_blog_image_list_height'=>260,'module_probg_blog_image_article_width'=>900,'module_probg_blog_image_article_height'=>600,'module_probg_blog_image_gallery_width'=>300,'module_probg_blog_image_gallery_height'=>220,'module_probg_blog_default_image'=>'','module_probg_blog_sitemap'=>1,'module_probg_blog_cache'=>1,'module_probg_blog_version'=>'0.10.0'));
        }
    }
    ...
    public function migrate() {
        $this->ensureCategorySchema();
        if ($this->config->get('module_probg_blog_version') === '0.10.0') return;
        ...
        foreach ($categories->rows as $row) {
            ...
            if (trim($row['meta_title']) === '') {
                $this->db->query("UPDATE `" . DB_PREFIX . "probg_blog_category_description` SET meta_title='" . $this->db->escape($title) . "' WHERE category_id='" . $category_id . "' AND language_id='" . $language_id . "'");
            }
        ...
        foreach ($articles->rows as $row) {
            ...
            $language_id = (int)$row['language_id'];
            if (trim($row['meta_title']) === '') {
                $this->db->query("UPDATE `" . DB_PREFIX . "probg_blog_article_description` SET meta_title='" . $this->db->escape($row['title']) . "' WHERE article_id='" . $article_id . "' AND language_id='" . $language_id . "'");
            }
        ...
        $settings['module_probg_blog_version'] = '0.10.0';
