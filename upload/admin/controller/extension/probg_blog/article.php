...
class ControllerExtensionProbgBlogArticle extends Controller {
    ...
    protected function getList() {
        ...
        $this->baseData($data);
        foreach (array('text_list','text_no_results','text_confirm','text_all','text_enabled','text_disabled','column_image','column_id','column_title','column_category','column_sort_order','column_status','column_date_added','column_date_modified','column_action','entry_title','entry_category','entry_status','entry_date_added_from','entry_date_added_to','button_add','button_delete','button_edit','button_filter') as $key) $data[$key] = $this->language->get($key);
    ...
    protected function getForm() {
        ...
        foreach (array(
            'text_add','text_edit','text_content','text_data','text_images','text_related','text_main_image','text_additional_images','text_enabled','text_disabled','text_automatic',
            'entry_title','entry_short_description','entry_description','entry_meta_title','entry_meta_description','entry_meta_keyword','entry_seo_keyword','entry_category','entry_image','entry_related','entry_sort_order','entry_status','entry_date_added','entry_date_modified',
            'help_seo_keyword','help_related','button_save','button_cancel','button_add_image','button_remove'
        ...
        $data['action'] = $this->url->link($article_id ? 'extension/probg_blog/article/edit' : 'extension/probg_blog/article/add', 'user_token=' . $this->session->data['user_token'] . ($article_id ? '&article_id=' . $article_id : '') . $this->listUrl(), true);
        ...
        $data['article_description'] = isset($this->request->post['article_description']) ? $this->request->post['article_description'] : ($article_id ? $this->model_extension_probg_blog_article->getArticleDescriptions($article_id) : array());

        $this->load->model('extension/probg_blog/category');
        $data['categories'] = $this->model_extension_probg_blog_category->getCategories(array('sort' => 'cd.title', 'order' => 'ASC'));
        $data['category_id'] = isset($this->request->post['category_id']) ? (int)$this->request->post['category_id'] : (isset($article_info['category_id']) ? (int)$article_info['category_id'] : 0);
        $data['image'] = isset($this->request->post['image']) ? $this->request->post['image'] : (isset($article_info['image']) ? $article_info['image'] : '');

        $this->load->model('tool/image');
        $data['placeholder'] = $this->model_tool_image->resize('no_image.png', 100, 100);
        $data['thumb'] = ($data['image'] && is_file(DIR_IMAGE . $data['image'])) ? $this->model_tool_image->resize($data['image'], 100, 100) : $data['placeholder'];

        $images = isset($this->request->post['article_image']) ? $this->request->post['article_image'] : ($article_id ? $this->model_extension_probg_blog_article->getArticleImages($article_id) : array());
        $data['article_images'] = array();
        foreach ($images as $image_info) {
            $image = isset($image_info['image']) ? $image_info['image'] : '';
            $data['article_images'][] = array('image'=>$image,'thumb'=>($image && is_file(DIR_IMAGE . $image)) ? $this->model_tool_image->resize($image, 100, 100) : $data['placeholder'],'sort_order'=>isset($image_info['sort_order']) ? (int)$image_info['sort_order'] : 0);
        }

        $related_ids = isset($this->request->post['article_related']) ? (array)$this->request->post['article_related'] : ($article_id ? $this->model_extension_probg_blog_article->getArticleRelatedProducts($article_id) : array());
        $data['product_relateds'] = array();
        $this->load->model('catalog/product');
        foreach ($related_ids as $product_id) {
            $product_info = $this->model_catalog_product->getProduct((int)$product_id);
            if ($product_info) $data['product_relateds'][] = array('product_id'=>(int)$product_info['product_id'],'name'=>$product_info['name']);
        }

        $data['sort_order'] = isset($this->request->post['sort_order']) ? (int)$this->request->post['sort_order'] : (isset($article_info['sort_order']) ? (int)$article_info['sort_order'] : 0);
        $data['status'] = isset($this->request->post['status']) ? (int)$this->request->post['status'] : (isset($article_info['status']) ? (int)$article_info['status'] : 1);
        $data['date_added'] = isset($article_info['date_added']) ? date($this->language->get('datetime_format'), strtotime($article_info['date_added'])) : $this->language->get('text_automatic');
        $data['date_modified'] = isset($article_info['date_modified']) ? date($this->language->get('datetime_format'), strtotime($article_info['date_modified'])) : $this->language->get('text_automatic');
    ...
    protected function validateForm() {
        if (!$this->user->hasPermission('modify', 'extension/probg_blog/article')) $this->error['warning'] = $this->language->get('error_permission');
        $this->load->model('extension/probg_blog/category');
        $category_id = isset($this->request->post['category_id']) ? (int)$this->request->post['category_id'] : 0;
        if (!$category_id || !$this->model_extension_probg_blog_category->getCategory($category_id)) $this->error['category'] = $this->language->get('error_category');

        $this->load->model('localisation/language');
        $article_id = isset($this->request->get['article_id']) ? (int)$this->request->get['article_id'] : 0;
        foreach ($this->model_localisation_language->getLanguages() as $language) {
            $language_id = (int)$language['language_id'];
            $value = isset($this->request->post['article_description'][$language_id]) ? $this->request->post['article_description'][$language_id] : array();
            ...
            if ($keyword !== '' && (!preg_match('/^[\p{L}\p{N}_-]+$/u', $keyword) || $this->model_extension_probg_blog_article->getSeoUrlByKeyword($keyword, $language_id, $article_id))) $this->error['seo_keyword'][$language_id] = $this->language->get('error_seo_keyword_exists');
        }
    ...
    private function baseData(&$data) {
        ...
        $data['breadcrumbs'] = array(
            ...
            array('text'=>$this->language->get('heading_title'),'href'=>$this->url->link('extension/probg_blog/article','user_token=' . $this->session->data['user_token'],true))
        );
    ...
    private function listUrl($exclude = array()) {
        ...
        foreach (array('filter_title','filter_category_id','filter_status','filter_date_added_from','filter_date_added_to','sort','order','page') as $parameter) {
            if (!in_array($parameter, $exclude, true) && isset($this->request->get[$parameter]) && $this->request->get[$parameter] !== '') $url .= '&' . $parameter . '=' . urlencode($this->request->get[$parameter]);
        }
        return $url;
    }
}
